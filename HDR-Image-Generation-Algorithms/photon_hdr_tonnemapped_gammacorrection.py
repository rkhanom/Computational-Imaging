# -*- coding: utf-8 -*-
"""Photon_HDR_Tonnemapped_Gammacorrection.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1E-3Ud4KWi7JD3crOtMuPPqWmi4MxQYsJ
"""

#The first program is for Photon weight function which is going to produce the Photon HDR image, Gaussian Tonned mapped image and Gaussian gamma-correctedmimage
import os
...
# calculating the weights
    ...
    photon_hdr = np.sum(weights_arr * scaled_image, axis=0) / (np.sum(weights_arr, axis=0) + epsilon)
    ...
    writeHDR("/Users/rk/Downloads/HDRProject/nef_images/tiff_images/results/{}_hdr_0.05_0.95.PNG".format(weight_name[weight_indx]), photon_hdr)

    print("Part3:Tonemapping")

    # Tone_mapping only for gaussian image

    h, w, ch= photon_hdr.shape

    tonemapp_photon_hdr= np.zeros_like(photon_hdr )

    K= 0.01
    B= 0.95
    # We will tone-mapusing the operator proposed by Reinhard

    I_m = np.exp(np.mean(np.log(photon_hdr[:, :, :] + epsilon)))
    I_tild_hdr = K * photon_hdr[:, :, :] / I_m
    I_tild_white = B * np.max(I_tild_hdr)
    tonemapp_photon_hdr[:, :, :] = (I_tild_hdr * (1 + I_tild_hdr / (I_tild_white ** 2))) / (1 + I_tild_hdr)


    writeHDR("/Users/rk/Downloads/HDRProject/nef_images/tiff_images/results/{}_hdr_tm_K_{}_B_{}.PNG".format(weight_name[weight_indx], K, B), tonemapp_photon_hdr)
    ...
    # Gamma correction
    gamma_photon_hdr= np.where(tonemapp_photon_hdr <= 0.0031308, 12.92 * tonemapp_photon_hdr, 1.055 * (tonemapp_photon_hdr ** (1/2.5)) - 0.055)

    writeHDR("/Users/rk/Downloads/HDRProject/nef_images/tiff_images/results/{}_hdr_tm_gamma_K_{}_B_{}.PNG".format(weight_name[weight_indx],K, B), photon_hdr)
