# -*- coding: utf-8 -*-
"""Uniform_HDR_Tonnedmapped_Gammacorrection.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1wo56Hay-CtdIVm6CJ7Enh9nwOyhTrS7J
"""

# This protion is goging to generate the HDR, tonnedmapped and gamma corrected  images for Uniform weight function
...
# calculating the weights
    ...
    uniform_hdr = np.sum(weights_arr * scaled_image, axis=0) / (np.sum(weights_arr, axis=0) + epsilon)
    # write the hdr image accoriding to the input
    #call helper function writeHDR to save the images

    writeHDR("/Users/rk/Downloads/HDRProject/nef_images/tiff_images/results/{}_hdr_0.05_0.95.PNG".format(weight_name[weight_indx]), uniform_hdr)
    ...
    h, w, ch= uniform_hdr.shape

    tonemapp_uniform_hdr= np.zeros_like(uniform_hdr )

    K= 0.01
    B= 0.95
    # We will tone-mapusing the operator proposed by Reinhard

    I_m = np.exp(np.mean(np.log(uniform_hdr[:, :, :] + epsilon)))
    I_tild_hdr = K * uniform_hdr[:, :, :] / I_m
    I_tild_white = B * np.max(I_tild_hdr)
    tonemapp_uniform_hdr[:, :, :] = (I_tild_hdr * (1 + I_tild_hdr / (I_tild_white ** 2))) / (1 + I_tild_hdr)


    writeHDR("/Users/rk/Downloads/HDRProject/nef_images/tiff_images/results/{}_hdr_tm_K_{}_B_{}.PNG".format(weight_name[weight_indx], K, B), tonemapp_uniform_hdr)
    ...
    # Gamma correction
    gamma_uniform_hdr= np.where(tonemapp_uniform_hdr <= 0.0031308, 12.92 * tonemapp_uniform_hdr, 1.055 * (tonemapp_uniform_hdr ** (1/2.5)) - 0.055)

    writeHDR("/Users/rk/Downloads/HDRProject/nef_images/tiff_images/results/{}_hdr_tm_gamma_K_{}_B_{}.PNG".format(weight_name[weight_indx],K, B), gamma_uniform_hdr)
